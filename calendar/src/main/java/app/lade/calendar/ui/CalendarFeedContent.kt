...
import app.lade.entry.ui.entryUnitLabel
...
private fun feedDetail(item: FeedItem, timeFmt: DateTimeFormatter): String? {
	...
	if (item.source == FeedSource.HABIT) {
		val status = entryMarkStatusLabel(item.habitResult)
		val unitGoal = item.habitGoalUnit
		val unit = if (unitGoal != null) entryUnitLabel(unitGoal) else null
		val unit = item.habitGoalUnit?.let { entryUnitLabel(it) }
