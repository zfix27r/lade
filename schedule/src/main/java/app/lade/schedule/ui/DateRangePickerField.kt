...
fun DateRangePickerField(
	...
	Column(modifier = modifier, verticalArrangement = Arrangement.spacedBy(gap)) {
		Row(
			...
			OutlinedButton(
				...
				Text(
					if (dateTo != null) {
						stringResource(R.string.format_date_to, dateTo.format(formatter))
					} else {
						stringResource(R.string.temporal_date_to_open)
					},
				)
			}
		}
		if (dateTo != null) {
			TextButton(onClick = { onDateToChange(null) }) {
				Text(stringResource(R.string.temporal_date_to_clear))
			}
		}
	}
	...
	if (pickingTo) {
		val initial = dateTo ?: dateFrom.plusMonths(1)
		val state = rememberDatePickerState(
