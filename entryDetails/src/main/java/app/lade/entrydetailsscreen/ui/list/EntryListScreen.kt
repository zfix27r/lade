...
import app.lade.entry.api.labels.entryKindLabel
			...
			FlowRow(
				...
				EntryKind.entries.forEach { kind ->
					FilterChip(
						...
						label = { Text(entryKindLabel(kind)) },
					)
				}
				FilterChip(
				...
				LazyColumn(modifier = Modifier.fillMaxSize()) {
					items(state.items, key = { it.id }) { entry ->
						ListItem(
							...
							supportingContent = { Text(entryKindLabel(entry.kind)) },
							trailingContent = {
								if (entry.isArchived) {
